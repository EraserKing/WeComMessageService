...
namespace Mikan.Services
{
    public class MikanBackgroundService : IHostedService, IDisposable
    {
        ...
        public async Task<string> Refresh()
        {
                        ...
                        case SyndicationElementType.Item:
                            ...
                            if (links != null)
                            {
                                if (!FetchedItems.Any(ci => ci.Title == item.Title))
                                {
                                    ...
                                    Logger.LogInformation("MIKAN: Received new item [{Key}]: {Title}, {Url}", newItem.Key, newItem.Title, newItem.Url);
                                }
                                else
                                {
                                    Logger.LogTrace("MIKAN: Received existing item {Title}", item.Title);
                                }

                            }
                            else
                            {
                                Logger.LogWarning("MIKAN: Received item without links: {Title}", item.Title);
                            }
                            break;

                        default:
                            Logger.LogTrace("MIKAN: Received unsupported feed of type {ElementType}", feedReader.ElementType);
                            break;
                    }
                }
            }
            ...
            if (newItems.Count > 0)
            {
                Logger.LogInformation("MIKAN: Found {ItemCount} items to send {HostInfo}", newItems.Count, string.IsNullOrEmpty(Options.Value.PublicHost) ? "without public host" : "with public host " + Options.Value.PublicHost);
        ...
        public async Task ClearOutDatedCache()
        {
            ...
            try
            {
                ...
                {
                    Logger.LogInformation("MIKAN: Remove {Title} due to out-dated", itemToClear.Title);
